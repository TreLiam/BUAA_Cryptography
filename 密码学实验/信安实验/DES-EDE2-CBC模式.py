...
def xor_str(s1,s2):
    ...
    res_hex_str=hex(res)[2:]
    res_hex_str="0"*(16-len(res_hex_str))+res_hex_str
    return res_hex_str
...
def decrypt(ciphertext,roundKey):
    ...
    return ciphertext
def encrypt_three(plaintext,k1,k2):
    mes = encrypt(plaintext, generatekey(k1))
    message = decrypt(mes, generatekey(k2))
    result = encrypt(message, generatekey(k1))
    return result
def decrypt_three(ciphertext,k1,k2):
    mes = decrypt(ciphertext, generatekey(k1))
    message = encrypt(mes, generatekey(k2))
    result = decrypt(message, generatekey(k1))
    return result
def DES(n,s,k1,k2,op,IV):
    if op == 1:
        x = (16 - (n - (n // 16) * 16)) // 2
        s = s + x * ("0" + hex(x)[2:])
        lengh = len(s) // 16
        res = toHex(encrypt_three(toBin(xor_str(s[0:16], IV)), k1, k2), 16)
        for i in range(lengh - 1):
            res += toHex(
                encrypt_three(toBin(xor_str(s[(i + 1) * 16:(i + 2) * 16], res[i * 16:(i + 1) * 16])), k1, k2),
                16)
        print(res)
    else:
        res = ""
        res = xor_str(toHex(decrypt_three(toBin(s[0:16]), k1, k2), 16), IV)
        lengh = n // 16

        for i in range(lengh - 1):
            res += xor_str(toHex(decrypt_three(toBin(s[(i + 1) * 16:(i + 2) * 16]), k1, k2), 16),
                           s[i * 16:(i + 1) * 16])
        # 去除填充位
        x = int(res[-2:], 16)
        res = res[:n - x * 2]
