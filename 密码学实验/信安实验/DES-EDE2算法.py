...
def DES(s,k1,k2,op):
    if op == 1:
        mes = encrypt(s,generatekey(k1))
        message = decrypt(mes,generatekey(k2))
        text = encrypt(message,generatekey(k1))
    else:
        mes = decrypt(s, generatekey(k1))
        message = encrypt(mes, generatekey(k2))
        text = decrypt(message, generatekey(k1))
...
if __name__ == '__main__':
    ...
    k2 = trans(k2)
    op = int(input())
    f4 = open('./3DES.txt',encoding='utf-8')
    content = f4.readline().strip().replace('\n', '').replace('\r', '')
